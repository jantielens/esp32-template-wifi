...
build_board() {
    ...
    if [[ -d "$board_override_dir" ]]; then
        ...
        # Force include path and board macro define; include file via app.ino guard
        board_macro="BOARD_${board_name^^}"
        EXTRA_FLAGS+=(--build-property "compiler.cpp.extra_flags=-I$board_override_dir -D$board_macro")
    else
